...
const ContratoList = ({ contratos, clientes, onAdd, onEdit, onDelete, visibleFields, onToggleViewOptions, onToggleReportModal, searchTerm, onSearchChange }: ContratoListProps) => {
  const [collapsedClients, setCollapsedClients] = useState<Record<string, boolean>>({});

  const toggleClientCollapse = (clientName: string) => {
    setCollapsedClients(prevState => ({
      ...prevState,
    ...
    <div>
      <div className="list-header">
        ...
        <div className="header-controls">
          ...
          />
          <button onClick={onAdd} className="add-btn">Nuevo Contrato</button>
      ...
      {Object.entries(groupedByClient).map(([clientName, clientContratos]) => (
        <div key={clientName}>
          <h3 className="client-group-header" onClick={() => toggleClientCollapse(clientName)} style={{ cursor: 'pointer' }}>
            {clientName} {collapsedClients[clientName] ? '▼' : '▲'}
          </h3>
